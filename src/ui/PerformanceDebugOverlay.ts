...
export default class PerformanceDebugOverlay extends Phaser.GameObjects.Text {
  ...
  private displayStats(): void {
    ...
    const totalCreated =
      poolStats.created.normal +
      poolStats.created.pierce +
      poolStats.created.explosive;
    const totalReused =
      poolStats.reused.normal +
      poolStats.reused.pierce +
      poolStats.reused.explosive;
    const reuseRate =
      totalCreated > 0 ? Math.round((totalReused / totalCreated) * 100) : 0;
