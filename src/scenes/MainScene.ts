...
import MemoryDebugOverlay from "../ui/MemoryDebugOverlay"; // NEW 👈

import Projectile from "../entities/Projectile";
import {
  EnemySpawnService,
  DEFAULT_WAVES,
} from "../services/EnemySpawnService";
...
export default class MainScene extends Phaser.Scene {
  ...
  // ========================================
  private memoryOverlay!: MemoryDebugOverlay;
  ...
  create(): void {
    Logger.info(
      "💡 Controls: WASD/Arrows=Move, Space=Shoot, Q=Switch Weapon, N=Next Wave"
    );
    Logger.info(
      "🔧 Debug: F1=General, F2=Collision, F3=Performance, F4=Memory"
    );
  ...
  private createGameEntities(): void {
    ...
    // Create projectile group with pooling
  ...
  private initializeServices(): void {
    ...
    const callbacks: CollisionCallbacks = {
      onProjectileHitEnemy: (e) => this.handleProjectileHitCallback(e),
      onEnemyHitPlayer: (e) => this.handleEnemyHitCallback(e),
    };

    // Initialize collision system
    this.collisionService.initialize(groups, callbacks);
  ...
  private setupUserInterface(): void {
    ...
    this.memoryOverlay = new MemoryDebugOverlay(this);
  ...
  private handleProjectileHitCallback(eventData: any): void {
    ...
    Logger.info(
      `🎯 MainScene: Projectile hit processed - advanced effects ready`
    );
  ...
  private startNextWave(): void {
    ...
    if (this.currentWaveIndex >= DEFAULT_WAVES.length) {
      Logger.info(
        "🎉 MainScene: All preset waves completed - starting endless mode"
      );
  ...
  private onWaveSpawningComplete(wave: number): void {
    Logger.info(
      `📈 MainScene: Wave ${wave} spawning finished - checking for remaining enemies`
    );
  ...
  private checkWaveCompletion(): void {
    ...
    if (aliveEnemies === 0 && !isSpawning) {
      if (!this.waveCompleteNotified) {
        Logger.info(
          "🏆 MainScene: Wave cleared! Ready for next wave (Press N)"
        );
  ...
  shutdown(): void {
    ...
    /* Overlays – MUST be cleaned to free pooled graphics */
    this.memoryOverlay?.destroy(); //  ←  NEW: now “read”, TS‑6133 gone
